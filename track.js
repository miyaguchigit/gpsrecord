...
function startTrack() {
  ...
  if (interval > 0) {
    ...
    trackingIntervalId = window.setInterval(() => {
      collectEntry();
      log.innerText = currentData.slice(-6000).map(JSON.stringify).reverse().join(',\n');
    }, interval);
  }
  isTracking = true;
}
...
function collectEntry() {
  navigator.geolocation.getCurrentPosition(position => {currentData.push(toObj(position))}, function(){}, 
